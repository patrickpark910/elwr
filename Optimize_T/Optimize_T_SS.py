...
def main():
    ...
    for c_0 in c_list: # mgLi6/kgU
        ...
        df.to_csv(f'output-SS-Li{str(c_0).zfill(3)}.csv')
...
def K_c(b,c):
    return K_0(b) + (1.279322E-06*c**2 - 1.326092E-03*c)*1 # .95
...
def plot_results(c, Tnd, Td, b):
    ...
    # plt.grid(True)
    plt.show()
    plt.savefig('Optimized_T_decay_SS.svg',format='svg') # ,bbox_inches=0,transparent=True)
